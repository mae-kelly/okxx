...
use rust_decimal::prelude::ToPrimitive;  // Added missing import
use std::sync::Arc;
...
impl FlashLoanManager {
    pub fn new(provider: Arc<Provider<Http>>) -> Self {
        let providers = vec![
            FlashLoanProvider {
                ...
                fee_percentage: Decimal::from_str_exact("0.0009").unwrap(), // 0.09%
    ...
    pub fn get_cheapest_provider(&self, token: &str, amount: Decimal) -> Option<&FlashLoanProvider> {
        self.providers
            ...
            .min_by_key(|p| {
                // Fixed: Using to_u64() with proper import
    ...
    pub async fn execute_flash_loan(
        &self,
        _provider: &FlashLoanProvider,  // Added underscore to suppress warning
        _token_address: &str,           // Added underscore to suppress warning
        _amount: Decimal,                // Added underscore to suppress warning
    ) -> Result<()> {
